...
def setFilePath(): 
    global filePath
    filePath = getFileConfig("TextFilePath")
...
def setMistake(msg):
    global mistake
    if msg is None: mistake = ""
    else: mistake = f'错误警告：{msg}'
...
# 定义变量
def init():
    setFilePath() # 设置当前编辑文件路径
    currentDir = getFileConfig("Root") # 获取当前文件的目录
    iconPath = os.path.join(currentDir, 'static', 'logo.ico') # 图标路径
    personalConfigPath = currentDir + r'\config\personalConfig.json' # 样式配置文件路径
    return iconPath, personalConfigPath # 返回图标路径和样式配置文件路径


# 定义主窗口类
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        iconPath, personalConfigPath = init() # 初始化全局变量
        ...
        # 创建一个图标对象``
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(iconPath), QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.Off) # 添加一个图像到图标对象中，使用"./Ui/logo.ico"路径下的图像，正常模式
        ...
        # 读取配置文件
        if os.path.exists(personalConfigPath): 
            try:
            ...
            except: messagebox.showerror("错误", "配置文件读取失败，请检查配置文件内容。(5604)")
        else: iniStyle()
    ...
    def onCursorPositionChanged(self):
        ...
        self.statusLabel.setText(f"行号: {cursorLineNumber}     {mistake}     文件：{filePath}     版本：v1.0.0") # 修改状态栏的文本
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.plainTextEdit.setPlainText(getText()) # 显示默认代码
        self.action.triggered.connect(lambda: (openFile(), self.plainTextEdit.setPlainText(getText()))) # 绑定打开文件操作
