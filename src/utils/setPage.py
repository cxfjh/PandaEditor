...
def reset_program():
    ...
    try:
        ...
        if messagebox.askquestion('重置程序', '确定要重置程序吗？') == 'yes':
            ...
            # 重启程序
            restart_program()
    except Exception as e: messagebox.showerror('重置程序', f'重置程序失败，错误信息：{str(e)}，请检查配置文件，或使用管理员身份运行程序！')
...
def uninstall_program():
    ...
    try:
        ...
        if messagebox.askquestion('卸载程序', '确定要卸载程序吗？') == 'yes':
            ...
            subprocess.run(uninstaller_path, check=True) # 执行卸载程序
...
def restart_program():
    ...
    for proc in psutil.process_iter(['pid', 'name']):
        if proc.info['pid'] == current_pid: proc.kill()
